  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    button.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View view) {
        ...
        if (!(param[1]<labyrinthMap.size() & param[3]<labyrinthMap.size() &
              param[0]<labyrinthMap.get(0).size() &
              param[2]<labyrinthMap.get(0).size())) {
          Toast.makeText(getApplicationContext(), "Неверные координаты", Toast.LENGTH_SHORT).show();
        }
        else if (labyrinth.get(param[2], param[3]).getWall() == 1 || labyrinth.get(param[0], param[1]).getWall() == 1) {
          Toast.makeText(getApplicationContext(), "Там стены", Toast.LENGTH_SHORT).show();
        }
        else {
          button.setText("Вычисление...");
          ...
          if (!route.isPathFound()) {
            ...
            tv.setText(String.valueOf(route.getTurn()));
            ...
            for (List<Point> y : labyrinthMap) {
              for (Point xPoint : y) {
                ...
                if (list.contains(xPoint)) {
                  labText += "<font color=#ffffff>"+wall+
                        "</font>";
                }
                else {
                  labText += "<font color=#999999>"+wall+"</font>";
                }
              }
              labText += "<br>";
            }
